/**
 * HUNTER - Lead Generation Agent
 * Proactive prospect identification and outreach automation
 */
...
export class HunterAgent extends BaseAgent {
    ...
    this.registerTool({
      ...
      parameters: {
        ...
        properties: {
          ...
          listingType: { type: "string", enum: ["for_lease", "for_sale", "recently_leased"], description: "Type of listing" },
  ...
  private async runDailyProspecting(): Promise<AgentOutput> {
    ...
    for (const prospect of qualifiedProspects.slice(0, 10)) { // Limit daily outreach
    ...
    this.log("info", "runDailyProspecting", `Completed: ${results.length} signals, ${qualifiedProspects.length} qualified`)
  ...
  private async processNewListing(data: Record<string, unknown>): Promise<AgentOutput> {
    ...
    if (prospect.score >= this.huntingConfig.qualificationThreshold) {
      ...
      await this.requestHandoff(
        "MAYA_SALES",
        "Qualified prospect from real estate listing",
        { prospect },
        "normal"
      )
    }
